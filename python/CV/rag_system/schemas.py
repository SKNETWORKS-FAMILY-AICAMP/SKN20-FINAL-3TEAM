...
class SpaceAnalysis(BaseModel):
    """개별 공간 분석 결과"""
    space_name: str = Field(description="공간명 (예: 거실, 침실1)", default="공간")
    space_type: str = Field(description="공간 타입 (예: 공통공간, 개인공간)", default="공통공간")
    ...
    evaluation_comment: str = Field(description="설계 평가 코멘트", default="")

class FloorPlanAnalysis(BaseModel):
    """전체 평면도 분석 결과"""
    image_name: str = Field(description="이미지 파일명", default="unknown")
    structure_type: str = Field(description="건축물 유형 (판상형/타워형/혼합형)", default="판상형")
    bay_count: int = Field(description="Bay 수", default=0)
    total_spaces: int = Field(description="전체 공간 수", default=0)
    room_count: int = Field(description="침실 수", default=0)
    bathroom_count: int = Field(description="욕실 수", default=0)
    balcony_ratio: float = Field(description="발코니 비율 (%)", default=0.0)
    windowless_ratio: float = Field(description="창 없는 공간 비율 (%)", default=0.0)
    ventilation_quality: str = Field(description="환기 품질 (우수/양호/보통/미흡)", default="보통")

    summary: str = Field(description="전체 평면도 요약", default="분석 결과가 불완전합니다.")
    spaces: List[SpaceAnalysis] = Field(description="공간별 분석", default=[])
