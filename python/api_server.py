...
class AnalyzeResponse(BaseModel):
    """도면 분석 응답 모델 - Spring Boot 백엔드와 호환되는 형식"""
    # 백엔드 PythonAnalysisResponse.java와 일치하는 필드명
    topology_json: str  # JSON 문자열로 변환하여 전달
    topology_image_url: str  # Base64 이미지 URL
...
async def analyze_floorplan(file: UploadFile = File(...)):
    ...
    try:
        ...
        # visualizer가 저장하는 실제 경로: temp_output/{file_stem}/topology_result.png
        ...
        # 10. 응답 생성 (Spring Boot 백엔드와 호환되는 형식)
        response = AnalyzeResponse(
            topology_json=json.dumps(topology_data, ensure_ascii=False),
            topology_image_url=topology_image_base64,
            **metrics,
